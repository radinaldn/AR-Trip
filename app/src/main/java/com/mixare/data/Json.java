 ...
 */
package com.mixare.data;

import org.json.JSONArray;
import org.json.JSONException;
import org.json.JSONObject;
import com.mixare.MixView;
...
public class Json extends DataHandler {
	...
	public void processAndroid10JSONObject(JSONObject jo) throws JSONException {
		...
		if (jo.has("lat") && jo.has("lng") && jo.has("title") && jo.has("streetAddress")) {
		...
		//	}
			...
			elevation = jo.getDouble("altitude");
			phonenumber = jo.getString("number");
			webpage = "http://maps.google.com/maps?daddr="+Double.toString(latitude)+","+Double.toString(longitude)+"&z=17";		

    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		conMgr = (ConnectivityManager) getSystemService(CONNECTIVITY_SERVICE);
		{
			if (conMgr.getActiveNetworkInfo() != null
				...
				Toast.makeText(getApplicationContext(), "Koneksi Internet Belum Anda Hidupkan, Aplikasi Tidak Berjalan Sempurna",
		...
		protected String doInBackground(String... args) {
			...
			try {
				...
				for(int i = 0; i < str_json.length(); i++){
					...
					if (d<1) {map.put(in_jarak,String.format("%.2f", gg)); map.put(miles, " M"); }
	                else  {map.put(in_jarak, String.format("%.2f", d)); map.put(miles, " KM"); }
                 ...
                 // Click event for single list row
                    list.setOnItemClickListener(new OnItemClickListener() {
            			...
            			public void onItemClick(AdapterView<?> parent, View view,
							...
							System.out.println("Isi kat = "+kat);



            				if (kat.equals("6")) {
            					Intent i = new Intent(ListActivity.this, WebViewActivity.class);
								i.putExtra("URL", URL);
								System.out.println("kategori adalah " +kat+ " maka buka URL BLOG");
            					startActivity(i);
							} // jika konten adalah wisata
							 else if (!kat.equals("6")) {
								...
								System.out.println("kategori adalah " +kat+ " maka buka activity");
								startActivity(in);
